...
description: >
  Copy YAP's bundled ralph_loop.py into a target project and wire up a `just`
  recipe to run it. Trigger on requests like "install the ralph loop in
  <path>", "vendor the ralph loop", or "install ralph loop" with no path
  given.
---

# Ralph loop install

Vendors `ralph_loop.py` (from the YAP skill) into the current repo so it can
be run and committed without depending on the skill install path.
...
2. **Locate the source `ralph_loop.py`.** Search the known candidate install
   paths (checking both project and home-relative forms):
   - `.claude/skills/YAP/scripts/ralph_loop.py`
   - `~/.claude/skills/YAP/scripts/ralph_loop.py`
   - `~/.cursor/skills/YAP/scripts/ralph_loop.py`
   - any other `*/YAP/scripts/ralph_loop.py` reachable from the repo root or
     home directory
